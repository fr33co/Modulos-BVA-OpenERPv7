...
class ir_attachment_employee(osv.Model):
    ...
    def _data_set(self, cr, uid, id, name, value, arg, context=None):
        ...
        if location:
            ...
            super(ir_attachment_employee, self).write(cr, SUPERUSER_ID, [id], {'store_fname': fname, 'file_size': file_size}, context=context)
        else:
            super(ir_attachment_employee, self).write(cr, SUPERUSER_ID, [id], {'db_datas': value, 'file_size': file_size}, context=context)
        return True

    _name = 'ir.attachment.employee'
    _columns = {
    ...
    def _auto_init(self, cr, context=None):
        super(ir_attachment_employee, self)._auto_init(cr, context)
        cr.execute('SELECT indexname FROM pg_indexes WHERE indexname = %s', ('ir_attachment_res_idx',))
        if not cr.fetchone():
            cr.execute('CREATE INDEX ir_attachment_res_idx ON ir_attachment_employee (res_model, res_id)')
            cr.commit()
    ...
    def check(self, cr, uid, ids, mode, context=None, values=None):
        ...
        if ids:
            if isinstance(ids, (int, long)):
                ids = [ids]
            cr.execute('SELECT DISTINCT res_model, res_id FROM ir_attachment_employee WHERE id = ANY (%s)', (ids,))
    ...
    def _search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ids = super(ir_attachment_employee, self)._search(cr, uid, args, offset=offset,
        ...
        cr.execute("""SELECT id, res_model, res_id FROM ir_attachment_employee WHERE id = ANY(%s)""", (list(ids),))
    ...
    def read(self, cr, uid, ids, fields_to_read=None, context=None, load='_classic_read'):
        ...
        return super(ir_attachment_employee, self).read(cr, uid, ids, fields_to_read, context, load)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'file_size' in vals:
            del vals['file_size']
        return super(ir_attachment_employee, self).write(cr, uid, ids, vals, context)

    def copy(self, cr, uid, id, default=None, context=None):
        self.check(cr, uid, [id], 'write', context=context)
        return super(ir_attachment_employee, self).copy(cr, uid, id, default, context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        if location:
            for attach in self.browse(cr, uid, ids, context=context):
                if attach.store_fname:
                    self._file_delete(cr, uid, location, attach.store_fname)
        return super(ir_attachment_employee, self).unlink(cr, uid, ids, context)

    def create(self, cr, uid, values, context=None):
        self.check(cr, uid, [], mode='write', context=context, values=values)
        if 'file_size' in values:
            del values['file_size']
        return super(ir_attachment_employee, self).create(cr, uid, values, context)
