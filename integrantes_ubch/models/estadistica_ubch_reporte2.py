...
def gen_est_centros_mun(cr, municipio):
	...
	cr.execute("SELECT count(*) as num_int FROM "+modelo_integrantes+" WHERE municipio = "+str(municipio))
	if cr.fetchone()[0] > 0:
		...
		for centros in cr.fetchall():
			...
			cr.execute("SELECT count(*) as num_int FROM "+modelo_integrantes+" WHERE municipio = "+str(municipio)+" AND centro_votacion = "+str(centros[0]))
			if cr.fetchone()[0] > 0:
		...
		#Nombre del archivo
		nombre_archivo = 'estadisticas_ubch_'+mun+'_'+time.strftime('%d-%m-%y')+'.pdf'
		#Ruta local
