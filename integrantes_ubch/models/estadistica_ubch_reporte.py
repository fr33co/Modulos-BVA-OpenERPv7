...
def gen_est_centros(cr, todo, municipio):
	...
	for centros in cr.fetchall():
		...
		if todo == True:
			...
			cr.execute("SELECT count(*) as F FROM "+modelo_integrantes+" WHERE centro_votacion = "+str(centros[0])+" AND sexo = '1'")
			F = cr.fetchone()[0]
			#Consultar todos los registros masculinos para el centro de votación especificado
			cr.execute("SELECT count(*) as M FROM "+modelo_integrantes+" WHERE centro_votacion = "+str(centros[0])+" AND sexo = '2'")
			M = cr.fetchone()[0]
