...
class bienes_nacionales(osv.Model):
	...
	_columns = {
		...
		'codigo':fields.many2one('codigos.gba', 'Código',required=True),
		...
		'ubicacion' : fields.many2one('stock.location', 'Ubicación Actual', required=False, readonly=True),
	...
	def on_change_codigo(self, cr, uid, ids, codigo, numero, name, nombre_des, serial, context=None):
		values = {}
		datos = self.pool.get('codigos.gba')
