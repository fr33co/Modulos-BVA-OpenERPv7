...
class adjunto_documento(osv.Model):
    ...
    def _data_set(self, cr, uid, id, name, value, arg, context=None):
        ...
        if location:
            ...
            super(adjunto_documento, self).write(cr, SUPERUSER_ID, [id], {'store_fname': fname, 'file_size': file_size}, context=context)
        else:
            super(adjunto_documento, self).write(cr, SUPERUSER_ID, [id], {'db_datas': value, 'file_size': file_size}, context=context)
        return True

    _name = 'adjunto.documento'
    ...
    _columns = {
        ...
        'res_model': fields.char('Modelo',size=64, readonly=True, help="The database object this attachment will be attached to"),
        'res_id': fields.integer('Referencia', readonly=True, help="The record id this is attached to"),
        ...
        'type': fields.selection( [ ('url','URL'), ('binary','Binary'), ],
                'Tipo', help="Binary File or URL", required=True, change_default=True),
        'url': fields.char('Url', size=1024),
        # al: We keep shitty field names for backward compatibility with document
        'datas': fields.function(_data_get, fnct_inv=_data_set, string='Archivo adjunto', type="binary", nodrop=True),
        'store_fname': fields.char('Archivo', size=256),
        'db_datas': fields.binary('Data'),
        'file_size': fields.integer('Espacio'),
        'item': fields.selection([('materiales','Materiales'),('cotizacion','Cotizacion'),('desc','Decendente'),('fechas','Fechas')], string="Mostrar en Forma", required=False),
    }
    ...
    def _auto_init(self, cr, context=None):
        super(adjunto_documento, self)._auto_init(cr, context)
        cr.execute('SELECT indexname FROM pg_indexes WHERE indexname = %s', ('ir_attachment_res_idx',))
        if not cr.fetchone():
            cr.execute('CREATE INDEX ir_attachment_res_idx ON adjunto_documento (res_model, res_id)')
            cr.commit()
    ...
    def check(self, cr, uid, ids, mode, context=None, values=None):
        ...
        if ids:
            if isinstance(ids, (int, long)):
                ids = [ids]
            cr.execute('SELECT DISTINCT res_model, res_id FROM adjunto_documento WHERE id = ANY (%s)', (ids,))
    ...
    def _search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=False, access_rights_uid=None):
        ids = super(adjunto_documento, self)._search(cr, uid, args, offset=offset,
        ...
        cr.execute("""SELECT id, res_model, res_id FROM adjunto_documento WHERE id = ANY(%s)""", (list(ids),))
    ...
    def read(self, cr, uid, ids, fields_to_read=None, context=None, load='_classic_read'):
        ...
        return super(adjunto_documento, self).read(cr, uid, ids, fields_to_read, context, load)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'file_size' in vals:
            del vals['file_size']
        return super(adjunto_documento, self).write(cr, uid, ids, vals, context)

    def copy(self, cr, uid, id, default=None, context=None):
        self.check(cr, uid, [id], 'write', context=context)
        return super(adjunto_documento, self).copy(cr, uid, id, default, context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        if location:
            for attach in self.browse(cr, uid, ids, context=context):
                if attach.store_fname:
                    self._file_delete(cr, uid, location, attach.store_fname)
        return super(adjunto_documento, self).unlink(cr, uid, ids, context)

    def create(self, cr, uid, values, context=None):
        self.check(cr, uid, [], mode='write', context=context, values=values)
        if 'file_size' in values:
            del values['file_size']
        return super(adjunto_documento, self).create(cr, uid, values, context)
