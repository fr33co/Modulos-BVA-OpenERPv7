...
class Gestion_reportes(osv.osv_memory):
    _name="gestion.filtro"
    ...
    def search_dep(self, cr, uid, ids, argument_search,item, context=None): # Proceso de busqueda de un manager(Gerente)
        ...
        if str(item) == "3":
            ...
            if not depart:
                mensaje = {
                        'title'   : "Busqueda por Ente / Adscrito",
                        'message' : "Disculpe, no se encuentran registros, intente de nuevo...",
                }
    ...
    def gestion_emitir_detallado(self, cr, uid, ids, context=None):
        ...
        if str(mostrar) == "todos": # CONDICIONAL PARA TODOS LOS REGISTROS
            ...
            cabezera = "Detallado Por "+str(inst).encode('UTF-8')+""
            item     = "departamento"
            if not campo['institucion']:
                raise osv.except_osv(_("Warning!"), _("Disculpe debe seleccionar el Ente Adscrito..."))
            else:
