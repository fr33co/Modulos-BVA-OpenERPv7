...
class Bancos(osv.Model):
	...
	def generar_codigo(self, cr, uid, ids, banco, context=None):
		...
		modelo_bancos = self.pool.get('tesoreria.banco')
		
		regs_bancos = modelo_bancos.browse(cr, uid, banco, context=context)
		
		cod_banco = regs_bancos.codigo
			
		print cod_banco
		
		
		#Generación del numerador 
		cr.execute('SELECT count(*) as num_cnt FROM tesoreria_cuenta WHERE tipo_banco ='+str(banco))
	...
	_columns = {
		...
		'tipo_banco': fields.many2one("tesoreria.banco", "Tipo de Banco", required = True, help="Seleccione el banco"),
